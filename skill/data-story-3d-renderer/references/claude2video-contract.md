...
`assets/stage-runtime/animations.jsx`, pre-bundled with React 18 into `assets/stage-runtime/stage-runtime.bundle.js`
and emitted by `scripts/build.py --stage-html`, satisfies every line:

| Requirement | Where it is met |
|---|---|
| `window.Stage` and `window.useTime` are functions before mount | entry script assigns both, then calls `createRoot` |
| Mount via `createRoot` into `#root` with rendered children | bundle entry; Stage renders its canvas div synchronously |
...
| Single self-contained `.html`, React inline, no JSX in plain scripts | pre-bundled runtime inlined by the Python builder; fonts as data URIs |
