...
def check_scene(s, publication, errs):
    ...
    for k in ('title_line_1', 'title_line_2'):
        if copy.get(k) and len(copy[k]) > 16: errs.append(f'{sid}: copy.{k} {copy[k]!r} is long (>16 chars); rewrite shorter rather than letting it shrink below 84 px')
    ...
    if t == 'population_ratio_10':
        ...
        bars = inp.get('bars', [])
        if not 2 <= len(bars) <= 4: errs.append(f'{sid}: ranking demonstrated layout takes 2–4 bars')
        ...
        for b in bars:
            ...
            if not b.get('country_code'): errs.append(f'{sid}: bar {b.get("label")} country_code missing (DE, FR, IT, GR, EU supported)')
        ...
        if 'value' in f and 'value' in to and round(to['value'] - f['value'], 6) != round(inp.get('delta_value', float('nan')), 6):
